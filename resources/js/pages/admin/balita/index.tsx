...
export default function BalitaIndex({ balita, breadcrumb, filter }: BalitaProps) {
    ...
    const submitSearch: FormEventHandler = (e) => {
        ...
        if (cleanedSearch.length > 0) {
            ...
            get(route('balita.index', { q: cleanedSearch, per_page: filter?.per_page, order_by: filter?.order_by }), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {},
            });
    ...
    useEffect(() => {
        ...
        if (cleanedOrderBy.length > 0) {
            // if search query is not empty, make request to server
            get(route('balita.index', { order_by: cleanedOrderBy, per_page: filter?.per_page, q: filter?.q }), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {},
            });
                                            ...
                                            <TableCell>
                                                <div className="flex flex-row items-center gap-2">
                                                    <DeleteConfirmationForm
                                                        title={`Hapus balita ${item.id}`}
                                                        id={item.id}
                                                        url={route('balita.destroy', { balita: item.id })}
                                                        setOpenDialog={setisDeleteDialog}
                                                    />
                                                    <Link href={route('balita.show', { balita: item.id })}>
                                                        <Button variant={'default'} type="button" className="bg-chart-1">
                                                            <EyeIcon size={4} />
                                                        </Button>
                                                    </Link>
                                                    <Link href={route('balita.edit', { balita: item.id })}>
